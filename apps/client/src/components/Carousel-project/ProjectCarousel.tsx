...
export const getNumVisibleSlides = (slides: string[], width: number) => {
  switch (true) {
    case width <= 1080:
      return 1;
    case width <= 1440:
...
export function ProjectCarousel({ text = {}, slides = [] }){
  ...
  return (
    <div className="projectPage__teams--container-item relative px-[10%]">
      ...
      <div className="projectPage__teams--carousel">
        <button
        className={activeIndex === 0 ? "hidden" : "absolute -left-10 md:left-0 p-2"}
        ...
        <button
         className={activeIndex >= slides.length - numVisibleSlides ? "hidden" : "absolute -right-10 lg:right-0 p-2"}
