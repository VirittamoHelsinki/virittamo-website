      ...
      <div id="work" className="flex flex-col gap-10">
        ...
        <div className="flex flex-col gap-2">
          ...
          <p className="text-[1.4rem] pt-10">
            ...
            >
              {jobseekerData.data.attributes.criterionLink}
            </a>
          </p>
          <p className="text-[1.4rem] pt-[4.375rem]">
        ...
        <ul id="services" className="">
          {jobseekerData.data.attributes.services.map((service, index) => (
            <li id={`v${index + 1}`} key={index} className="pt-[3rem]">
              <h1 className="text-[2rem] pt-[3rem]">{service.title}</h1>
              <p className="text-[1.675rem] pt-[2rem]">{service.description}</p>
            </li>
          ))}
        </ul>
      </div>
